#!/usr/bin/env bash
# M5 full redeploy: veil_core (with VkId::Swap/BatchSettle/AddLiquidity/RemoveLiquidity) + amm_pool
...
for vk_name in deposit kyc_credential transfer withdraw swap batch_settle add_liquidity remove_liquidity; do
    ...
    case $vk_name in
        ...
        add_liquidity) vk_id='{"AddLiquidity":[]}'   ; f=vk_add_liquidity ;;
        remove_liquidity) vk_id='{"RemoveLiquidity":[]}' ; f=vk_remove_liquidity ;;
    esac
...
# In M5, also initialize reserves
...
echo ""
echo "=== M5 deploy complete ==="
echo "  ASP=\$ASP"
echo ""
echo "Run e2e test:"
echo "  source deployments/testnet.env && node e2e-tests/src/m5_e2e.test.js"
