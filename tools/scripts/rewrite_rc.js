...
function updateFile(file) {
    ...
    let endIdx = content.indexOf('    fn poseidon2', startIdx);
    if (endIdx === -1) endIdx = content.indexOf('pub fn poseidon2', startIdx);
...
${constantsSrc}
        ...
        for r in 0usize..65 {
            ...
            row.push_back(parse_u256_hex(env, flat[r * 3]));
            row.push_back(parse_u256_hex(env, flat[r * 3 + 1]));
            row.push_back(parse_u256_hex(env, flat[r * 3 + 2]));
            rc.push_back(row);
        }
        rc
    }
...
updateFile('contracts/asp/src/lib.rs');
// For veil_core, the parse fn is called u256_from_hex64 or something else, wait!
