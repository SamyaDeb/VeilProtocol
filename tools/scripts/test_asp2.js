...
function toStruct(obj) {
    ...
    const fields = keys.map(k => new xdr.ScMapEntry({ key: xdr.ScVal.scvSymbol(k), val: obj[k] }));
...
async function main() {
    ...
    console.log("Expected computed_approved:", approvedTree.root.toString(16));
    console.log("credLeaf:", credLeaf.toString(16));
    ...
    try {
        ...
        if (sim.events) {
            sim.events.forEach(e => console.log(e.topics(), e.data()));
        }
    } catch(e) {
